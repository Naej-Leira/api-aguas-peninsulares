 ...
 */
function p(v: any): string {
  return Array.isArray(v) ? String(v[0]) : String(v);
}
function q(v: any, def: string): string {
  const s = p(v);
...
async function getPrimaryKeyColumn(table: AllowedTable): Promise<string> {
  ...
  if (!rows?.length) return "id"; // fallback
  return rows[0].COLUMN_NAME;
}

async function getTableColumns(table: AllowedTable): Promise<string[]> {
  const [rows]: any = await pool.query(
    `
    SELECT COLUMN_NAME
...
// ✅ GET /admin/tables
export async function adminListTables(_req: Request, res: Response) {
...
export async function adminList(req: Request, res: Response) {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT * FROM \`${table}\` ORDER BY \`${pk}\` ${order} LIMIT ? OFFSET ?`,
      [limit, offset]
    );
...
// ✅ GET /admin/:table/:id
export async function adminGetOne(req: Request, res: Response) {
  try {
    const table = p(req.params.table);
