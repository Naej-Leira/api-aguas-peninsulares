...
router.get("/admin", requireRole("ADMIN"), async (_req, res) => {
  try {
    ...
    const [[facturasHoy]] = await pool.query<any[]>(
      "SELECT COUNT(*) AS total FROM facturas WHERE DATE(fecha) = CURDATE() AND estado <> 'ANULADA'"
    );

    const [[ventasHoy]] = await pool.query<any[]>(
      "SELECT COALESCE(SUM(total),0) AS total FROM facturas WHERE DATE(fecha) = CURDATE() AND estado <> 'ANULADA'"
    );

    // Por cobrar (si estado es null o no PAGADO/CANCELADO)
    const [[porCobrar]] = await pool.query<any[]>(
      `
    ...
    // Ingresos de HOY
    const [[ingresosHoy]] = await pool.query<any[]>(
      "SELECT COALESCE(SUM(monto),0) AS total FROM ingresos WHERE DATE(fecha) = CURDATE()"
    );

    // Stock planta (sum stock de inventario_ubicacion en PLANTA)
    const [[stockPlanta]] = await pool.query<any[]>(
      "SELECT COALESCE(SUM(stock),0) AS total FROM inventario_ubicacion WHERE ubicacion = 'PLANTA'"
    );

    // Stock camiones (sum cantidad de stock_camion)
    const [[stockCamiones]] = await pool.query<any[]>(
      "SELECT COALESCE(SUM(cantidad),0) AS total FROM stock_camion"
    );

    // Transferencias HOY
    const [[transferenciasHoy]] = await pool.query<any[]>(
      "SELECT COUNT(*) AS total FROM transferencias WHERE DATE(fecha) = CURDATE()"
    );

    // Devoluciones HOY
    const [[devolucionesHoy]] = await pool.query<any[]>(
      "SELECT COUNT(*) AS total FROM devoluciones_camion WHERE DATE(fecha) = CURDATE()"
    );
 ...
 */
router.get("/camion", requireRole("CAMION"), async (req: any, res) => {
  try {
    ...
    const [[inventario]] = await pool.query<any[]>(
      "SELECT COUNT(*) AS items FROM inventario_camion"
    );

    // Stock del camión (sum cantidad)
    const [[stockCamion]] = await pool.query<any[]>(
      "SELECT COALESCE(SUM(cantidad),0) AS total FROM stock_camion WHERE camion_id = ?",
      [camionId]
    );

    // Facturas del día hechas por este usuario (usuario_id en facturas)
    const [[facturasHoy]] = await pool.query<any[]>(
      "SELECT COUNT(*) AS total FROM facturas WHERE usuario_id = ? AND DATE(fecha) = CURDATE() AND estado <> 'ANULADA'",
      [userId]
    );

    const [[ventasHoy]] = await pool.query<any[]>(
      "SELECT COALESCE(SUM(total),0) AS total FROM facturas WHERE usuario_id = ? AND DATE(fecha) = CURDATE() AND estado <> 'ANULADA'",
      [userId]
    );

    // Caja del día (caja_camion.fecha es DATE)
    const [[cajaHoy]] = await pool.query<any[]>(
      "SELECT * FROM caja_camion WHERE usuario_id = ? AND fecha = CURDATE() ORDER BY id DESC LIMIT 1",
      [userId]
    );
...
router.get("/planta", requireRole("PLANTA"), async (_req, res) => {
  try {
    ...
    // Movimientos hoy
    const [[movsHoy]] = await pool.query<any[]>(
      "SELECT COUNT(*) AS total FROM movimientos_stock WHERE DATE(fecha) = CURDATE()"
    );
